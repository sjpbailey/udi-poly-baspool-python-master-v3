...
class PoolNode(udi_interface.Node):
    ...
    def start(self):
        if self.ipaddress is not None:
            ...
            if self.bc.ePlatform == Platform.BASC_NONE:
                ...
                LOGGER.info(str(self.bc.uiQty) + ' Universal inputs in this BASpi Pool')
                LOGGER.info(str(self.bc.boQty) + ' Binary outputs in this BASpi Pool')    
    ...
    def pumpSpd(self, command):
        GV13 = int(command.get('value',)) #self.pumpOn = int(command.get('value',))
        if GV13 == 1:
    ...
    def cmdVlv(self, command):
        GV14 = int(command.get('value',))  #self.valveOn = int(command.get('value',))
        if GV14 == 1:
